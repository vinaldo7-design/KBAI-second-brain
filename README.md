...
vault — a *typed graph*, a real retrieval algorithm, and three cognitive
"lenses" that can disagree with each other in parallel. It is not a chatbot. It
is not built to reassure you; it is built to test you — to make you think with
your own writing as the prior.
...
1. [Why this exists](#why-this-exists) — the shape of a thought
2. [Yes, it's another second brain — here's why it's not Karpathy's](#yes-its-another-second-brain)
3. [The idea in one minute](#the-idea-in-one-minute)
4. [Watch it think](#watch-it-think) — two annotated transcripts
5. [How it works](#how-it-works) — architecture & the retrieval pipeline
6. [The typed graph](#the-typed-graph-meaning-that-travels)
7. [Cognitive profiles & Council Mode](#cognitive-profiles--council-mode)
8. [Show me the numbers](#show-me-the-numbers-the-eval-harness) — the eval harness
9. [Governance as architecture](#governance-as-architecture)
10. [Quick start](#quick-start)
11. [Reference](#reference) — commands, voices, layout
...
`cognitive-fit-theory` — matching the representation to the structure of the
problem is epistemology, not decoration.)

**An ordinary knowledge graph is flat.** Link-based tools (vanilla Obsidian,
Roam) fix the sequence problem — page 1 *can* now touch page 40 — but they trade
it for a subtler one: every link means the same thing. A line from a claim to its
...
things connect and hides *how* — which is the only part worth knowing.
...
Here is the most honest sentence in this README. **Making a machine think in human
shapes costs money and time.** A flat pile of vectors is cheap, fast, and
...
itself — costs tokens and latency a dumber system never pays.

The whole project is an attempt to **introduce human cognition into a machine
system** without falling into either ditch: too machine and it's a soulless search
...
*minimum capable system* — and the line it walks is the line between a mind and a
machine.
...
We argue about **what happens after synthesis.** Karpathy built a beautifully
organised library and handed the LLM a library card. This system does something
different with the books.
...
| **Stance** | dissent is *ad hoc* — you must prompt for it | dissent is *structural* — a skeptic lens with a `seek` policy + typed `contradicts` edges surface it unbidden (see [Demo 2](#watch-it-think)) |
...
be yours — and instead spends its effort making you *disagree with them*. A
second brain that only ever agrees with you is just a faster echo. This one has
a skeptic with a seat at the table.

There's a small joke buried in the architecture, too. The vault contains a note
called `knowledge-compression-translation-problem` —
...
— which is to say: Vinay wrote the theory of Karpathy's synthesis layer into the
vault, then built a graph that retrieves it. And `KBAI` literally stands for
*Knowledge-Based AI*. So "explain AI using a KBAI system" is not a slogan; it's
a description of the next section.
...
The bottleneck was never storage. It's *finding the right three notes — including
...
graph: a *knowledge-based AI* doing knowledge-based reasoning over a scholar's
own notes.
...
post-hoc introspection). Nothing clever yet — a vector search would find that
too. Then the **graph walk does what similarity can't**: it follows your typed
edges across a domain boundary.
...
stake in the verdict. That is the seat at the table the agreement-shaped systems
don't have, and it is exactly the kind of move a business-school class on AI
governance is trying to teach a human to make.
...
1. **Hybrid seed — find the starting notes two ways at once.** A *dense* vector
   search (the `BAAI/bge-small-en-v1.5` embedding model — ~33M parameters, ~130 MB
   on disk — running on your CPU; cosine similarity over note summaries) catches
   *meaning*; a *sparse* BM25
   keyword search (SQLite FTS5) catches *exact terms* the embedding might blur.
   The two ranked lists are merged with **Reciprocal Rank Fusion** (RRF, k=60) —
   a parameter-free way to combine rankings that rewards notes both methods like.
   If there's no keyword index, it degrades gracefully to dense-only.
...
2. **Route — don't use a sledgehammer on a thumbtack.** A small regex classifier
   ...
   redundant context on single-hop fact lookups.
   Anything relational ("*vs*", "*how*", "*why*", "*compare*", "*and*") gets the
   full walk. Reasoning is the safe default; the fast path is the exception it
   has to *earn*.
...
4. **Drop the map notes.** Index/"Map of Content" hub notes are wonderfully
...
7. **Assemble + attribute.** Finally, the system loads note content under a
...
|---|---|---|---|---|
| `explorer` | `analogous-to` ×1.6, `exemplifies` ×1.4 | — | allow · ignore | *"What else is shaped like this?"* Boosts the only two edges that cross domain boundaries — analogy and instantiation — so the walk leaves its home topic. Allows `contradicts` (it *notices* tensions) without hunting them. |
...
| `default` | — | — | suppress · ignore | Balanced fallback. Hides `contradicts` and weak `mentioned` ties; the neutral baseline. |
| `exhaustive` | — | — | allow · **exhaustive** | *"Show me everything, including the weak ties."* Admits `mentioned` and `contradicts` so the walk reaches notes every other lens gates out. Built for audits, not answers. |
...
contradiction (`contradicts`), qualification (`challenges`) — and a cognitive
profile is simply a *weighting over those moves*. That is why adding a lens needs
**no new edge types**, and why an eventual person-named profile will be *learned as
a weighting from usage*, never hand-coded.

> **Honest footnote (the kind a professor checks):** the edge-weight multipliers
> and the contradiction/mention policies are live in the PPR walk today. Two
> further fields — `path_length_preference` and `abstraction_preference` — are
> declared design intent: passed through but *not yet consumed* by the walk
...
- **MRR** (Mean Reciprocal Rank) — *how high does the first right answer land?*
  If the first relevant note is at rank 1 you score 1.0; rank 2 → 0.5; rank 4 →
  0.25. Rewards getting the best note to the *top*.
- **recall@10** — *of all the notes that should appear, what fraction made the
  top 10?* Measures coverage.
...
**never by running the retriever.** Building a golden set from your own
retriever's output is circular: you'd be grading the system against its own
opinion and congratulating yourself. By grounding labels in content the retriever
never sees, the benchmark can actually catch the retriever being *wrong*. It's
labelled "silver," not "gold" — the 10 labels are Claude-generated, not yet
expert-curated — and built to be upgraded note by note.

### The result: MRR 0.69 → 0.88, with the bruises showing
...
value.** The query router that followed was, by measurement,
quality-neutral: a pure efficiency play on a concept-heavy vault, shipped
*because* the eval proved it cost nothing.
...
(module ↔ storage), **L3** contract (every MCP tool result validated against a
...
This is the part that should interest anyone teaching AI *governance* rather than
AI *capability* — and it is the sharpest break from the "let the agent maintain
...
Restart Claude Desktop after edits — the MCP tool list is cached at process
start. Slash commands and voices live canonically in `~/.claude/commands/` and
...
`/pressure-test <note>` (auto-escalates challenge → council)
...
the essays are the published artefacts. This README is, itself, an exercise in
the thesis: explain the most, build the least, measure all of it.
